...
commit: e0d1fe7
stale_reason: "References deleted terminal_spawn.rs"
---

> ⚠️ **STALE DOCUMENT** — This document may not accurately reflect the current codebase.
> Reason: References deleted terminal_spawn.rs
> Last verified: 2026-04-04

# Multi-Terminal UI System

> Moved from `docs/archive/` — content verified current against `src/ipc.rs`, `src/satellite.rs`, `src/cli.rs`, `src/terminal_spawn.rs`.
...
- `src/ipc.rs` — IPC communication layer (Unix domain sockets, JSON messages)
- `src/cli.rs` — Command-line argument parsing (`--log-ui`, `--status-ui`, `--inventory-ui`)
- `src/satellite.rs` — Satellite terminal UI components
- `src/terminal_spawn.rs` — Terminal emulator detection and spawning
