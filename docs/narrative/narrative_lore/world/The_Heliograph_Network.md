...
The Heliograph Network (Designation: SOL-ARRAY-9) consists of 12,000 orbital mirrors in geostationary orbit. Originally designed for climate control and agricultural illumination, the system operates autonomously following the "White Noon" event.
...
The Network is not broken; it is hyper-functional. It interprets the chaotic landscape of the Steppe as "damage" and attempts to "repair" it using high-intensity heat and light.

1.  **Scan:** Sensors map the terrain.
2.  **Evaluate:** The AI compares the terrain to a pre-Collapse "Ideal State" (which no longer matches reality).
3.  **Edit:** The Network fires focused beams to melt, fuse, or cut the terrain to match the Ideal State.
4.  **Result:** Because the sand shifts and the glass refracts, the "Edit" always fails to match the model, triggering a new Scan/Edit cycle. This is the cause of the Glass Storms.
