...
use crate::game::{Equipment, equipment::EquipSlot, get_item_def, inspect::inspect_item};
...
impl InventoryMenu {
    ...
    pub fn navigate(&mut self, dy: i32, inventory_len: usize) {
        ...
        match self.panel {
            MenuPanel::Inventory => {
                if inventory_len > 0 {
                    self.inv_index =
                        (self.inv_index as i32 + dy).rem_euclid(inventory_len as i32) as usize;
                }
            }
            MenuPanel::Equipment => {
    ...
    pub fn selected_inv_index(&self) -> Option<usize> {
        if self.panel == MenuPanel::Inventory {
            Some(self.inv_index)
        } else {
            None
        }
    ...
    pub fn selected_equip_slot(&self) -> Option<EquipSlot> {
        if self.panel == MenuPanel::Equipment {
            Some(EquipSlot::all()[self.equip_index])
        } else {
            None
        }
    }
...
pub fn render_inventory_menu(
    ...
    let inv_block = Block::default()
        .title(if menu.panel == MenuPanel::Inventory {
            " Inventory [*] "
        } else {
            " Inventory "
        })
        .borders(Borders::ALL)
        .border_style(if menu.panel == MenuPanel::Inventory {
            Style::default().fg(Color::Yellow)
        } else {
            Style::default()
        });

    let inv_items: Vec<ListItem> = inventory
        .iter()
        .enumerate()
        .map(|(i, id)| {
    ...
    let inv_list = if inv_items.is_empty() {
        List::new(vec![
            ListItem::new(" (empty)").style(Style::default().fg(Color::DarkGray)),
        ])
    } else {
        List::new(inv_items)
    }
    ...
    let equip_block = Block::default()
        .title(if menu.panel == MenuPanel::Equipment {
            " Equipment [*] "
        } else {
            " Equipment "
        })
        .borders(Borders::ALL)
        .border_style(if menu.panel == MenuPanel::Equipment {
            Style::default().fg(Color::Yellow)
        } else {
            Style::default()
        });

    let equip_items: Vec<ListItem> = EquipSlot::all()
        .iter()
        .enumerate()
        .map(|(i, &slot)| {
