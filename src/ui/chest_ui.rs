    ...
    pub fn move_selection(&mut self, direction: i32) {
        match self.selected_panel {
            ChestPanel::ChestInventory => {
                ...
                self.chest_list_state.select(Some(current.saturating_add_signed(direction as isize)));
            }
            ChestPanel::PlayerInventory => {
                let current = self.inventory_list_state.selected().unwrap_or(0);
                self.inventory_list_state.select(Some(current.saturating_add_signed(direction as isize)));
            }
        }
    }
...
pub fn render_chest_ui(
    ...
    // Chest inventory
    let chest_items: Vec<ListItem> = chest.inventory.iter()
        .enumerate()
        .map(|(i, item)| {
    ...
    // Player inventory
    let inventory_items: Vec<ListItem> = player_inventory.iter()
        .enumerate()
        .map(|(i, item_id)| {
    ...
    frame.render_stateful_widget(inventory_list, main_chunks[1], &mut ui_state.inventory_list_state);
