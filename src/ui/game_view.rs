...
use crate::{GameState, Tile, VisualEffect, get_enemy_effects, get_light_def};
use ratatui::{
    prelude::*,
...
pub fn render_damage_numbers(frame: &mut Frame, area: Rect, state: &GameState) {
    ...
    for dn in &state.damage_numbers {
        ...
        if screen_x >= 0 && screen_x < view_w && screen_y >= 0 && screen_y < view_h {
            ...
            let rect = Rect::new(
                inner.x + screen_x as u16,
                inner.y + screen_y as u16,
                text.len() as u16,
                1,
            );
            frame.render_widget(
...
fn render_tile(
        ...
        for effect in player_effects {
            match effect {
                ...
                VisualEffect::Shimmer { speed, colors } => {
                    let color_idx = ((frame_count / *speed as u64) + (x as u64 ^ y as u64))
                        % colors.len() as u64;
                    let light = state.get_light_level(x as i32, y as i32);
                ...
                VisualEffect::Drift { speed, color } => {
                    let drift_phase =
                        ((frame_count / *speed as u64) + (x as u64 * 3 + y as u64 * 7)) % 10;
                    if drift_phase < 3 {
                        let light = state.get_light_level(x as i32, y as i32);
                        style = style.fg(dim_color(*color, light));
                    }
    ...
    if let Some(&ni) = state.npc_positions.get(&(x as i32, y as i32)) {
        ...
        if visible {
            ...
            return (
                npc.glyph(),
                Style::default().fg(dim_color(Color::Green, light)).bold(),
            );
        } else if revealed {
            return (
                state.map.tiles[idx].glyph(),
    ...
    if let Some(items) = state.item_positions.get(&(x as i32, y as i32)) {
        ...
        if visible {
            ...
            return (
                item.glyph(),
                Style::default().fg(dim_color(Color::LightMagenta, light)),
            );
        } else if revealed {
            return (
                state.map.tiles[idx].glyph(),
...
pub fn render_debug_console(frame: &mut Frame, console: &super::input::DebugConsole) {
    ...
    let height = ((area.height as f32 * 0.33) as u16)
        .max(8)
    ...
    let clear_lines: Vec<Line> = (0..height)
        .map(|_| Line::from(clear_text.clone()))
        .collect();
    ...
    if !console.history.is_empty() {
        ...
        let history_paragraph =
            Paragraph::new(history_lines).style(Style::default().fg(Color::Gray));
    ...
    let input_paragraph = Paragraph::new(prompt).style(Style::default().fg(Color::Green));
