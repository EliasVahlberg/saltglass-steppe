...
use crate::game::{GameState, get_recipe, all_recipe_ids, can_craft, get_item_def};
...
pub fn render_crafting_menu(frame: &mut Frame, menu: &CraftingMenu, state: &GameState) {
    ...
    let items: Vec<ListItem> = recipe_ids.iter().map(|id| {
    ...
    if let Some(recipe_id) = menu.selected_recipe_id() {
        if let Some(recipe) = get_recipe(recipe_id) {
            ...
            let mut lines = vec![
                Line::from(Span::styled(&recipe.name, Style::default().fg(Color::Yellow).bold())),
                Line::from(""),
            ...
            for (item_id, &required) in &recipe.materials {
                ...
                let item_name = get_item_def(item_id).map(|d| d.name.as_str()).unwrap_or(item_id);
                let color = if have >= required { Color::Green } else { Color::Red };
            ...
            let output_name = get_item_def(&recipe.output).map(|d| d.name.as_str()).unwrap_or(&recipe.output);
            lines.push(Line::from(format!("  {}x {}", recipe.output_count, output_name)));
            
            lines.push(Line::from(""));
            if craftable {
                lines.push(Line::from(Span::styled("Press Enter to craft", Style::default().fg(Color::Green))));
            } else {
                lines.push(Line::from(Span::styled("Missing materials", Style::default().fg(Color::Red))));
            }
