...
use crate::game::{
    skills::{get_skill_def, get_skills_by_category, get_abilities_by_category, SkillCategory, calculate_skill_cost},
    state::GameState,
};
...
impl SkillsMenu {
    ...
    pub fn upgrade_skill(&self, game_state: &mut GameState) -> Result<(), String> {
        let skill_id = self.get_selected_skill()
    ...
    pub fn use_ability(&self, game_state: &mut GameState) -> Result<(), String> {
        let ability_id = self.get_selected_ability()
            .ok_or("No ability selected")?;
...
pub fn render_skills_menu(f: &mut Frame, game_state: &GameState, menu: &SkillsMenu) {
    ...
    let header = Paragraph::new(format!("{} - {}", mode_text, category_text))
        .block(Block::default().borders(Borders::ALL).title("Character Development"))
    ...
    let footer_text = match menu.mode {
        SkillsMenuMode::Skills => "↑↓: Navigate | ←→: Category | Tab: Abilities | Enter: Upgrade | Esc: Close",
        SkillsMenuMode::Abilities => "↑↓: Navigate | ←→: Category | Tab: Skills | Enter: Use | Esc: Close",
    };
...
fn render_skills_list(f: &mut Frame, game_state: &GameState, menu: &SkillsMenu, area: Rect) {
    ...
    let items: Vec<ListItem> = skills.iter().map(|def| {
        ...
        let text = format!("{} (Lv.{}/{}) - {} SP", def.name, level, def.max_level, cost);
...
fn render_abilities_list(f: &mut Frame, game_state: &GameState, menu: &SkillsMenu, area: Rect) {
    ...
    let items: Vec<ListItem> = abilities.iter().map(|def| {
...
fn render_skill_details(f: &mut Frame, game_state: &GameState, menu: &SkillsMenu, area: Rect) {
    ...
    let content = if let Some(def) = skills.get(menu.selected_index) {
        ...
        vec![
            Line::from(vec![Span::styled(&def.name, Style::default().fg(Color::Yellow))]),
            Line::from(""),
...
fn render_ability_details(f: &mut Frame, game_state: &GameState, menu: &SkillsMenu, area: Rect) {
    ...
    let content = if let Some(def) = abilities.get(menu.selected_index) {
        ...
        if let Some(skill_def) = get_skill_def(&def.required_skill) {
            lines.push(Line::from(format!("Requires: {} Lv.{}", skill_def.name, def.required_level)));
            lines.push(Line::from(format!("Your Level: {}", skill_level)));
        }
        
        if !unlocked {
            lines.push(Line::from(vec![Span::styled("LOCKED", Style::default().fg(Color::Red))]));
        } else if *cooldown > 0 {
            lines.push(Line::from(vec![Span::styled(format!("Cooldown: {} turns", cooldown), Style::default().fg(Color::Red))]));
        }
