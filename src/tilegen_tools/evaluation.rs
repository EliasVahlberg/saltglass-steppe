...
use crate::tilegen_tools::{ConnectivityAnalysis, ConstraintResult, TileDistribution};
...
pub fn calculate_quality_score(
    ...
    let constraint_score =
        constraints.iter().filter(|c| c.passed).count() as f32 / constraints.len() as f32;
...
fn calculate_balance_score(distribution: &TileDistribution) -> f32 {
    let floor_ratio =
        *distribution.counts.get("floor").unwrap_or(&0) as f32 / distribution.total as f32;
    let wall_ratio =
        *distribution.counts.get("wall").unwrap_or(&0) as f32 / distribution.total as f32;

    // Ideal ratios: 40-60% floor, 30-50% wall
    let floor_score = if floor_ratio >= 0.4 && floor_ratio <= 0.6 {
        1.0
    } else {
        0.5
    };
    let wall_score = if wall_ratio >= 0.3 && wall_ratio <= 0.5 {
        1.0
    } else {
        0.5
    };
