...
impl MazeGenerator {
    ...
    fn get_unvisited_neighbors(&self, x: u32, y: u32, grid: &[Vec<CellState>], width: u32, height: u32) -> Vec<(u32, u32)> {
    ...
    fn add_random_loops(&self, grid: &mut [Vec<CellState>], width: u32, height: u32, rng: &mut ChaCha8Rng) {
        for y in 1..height - 1 {
            for x in 1..width - 1 {
                let random_val = rng.r#gen::<f32>();
                if grid[y as usize][x as usize] == CellState::Wall && random_val < self.params.loop_probability {
    ...
    fn grid_to_world_coords(&self, grid: &[Vec<CellState>], bounds: Rectangle, cell_size: u32) -> Vec<(u32, u32)> {
                ...
                if cell == CellState::Floor {
                    ...
                    for dy in 0..cell_size {
                        for dx in 0..cell_size {
                            if world_x + dx < bounds.x + bounds.width && world_y + dy < bounds.y + bounds.height {
