use crate::game::generation::structures::{Rectangle, Room, Corridor};
...
impl BSPAlgorithm {
    ...
    fn split_bounds(&self, bounds: Rectangle, horizontal: bool, rng: &mut ChaCha8Rng) -> (Rectangle, Rectangle) {
        ...
        if horizontal {
            ...
            let right = Rectangle::new(bounds.x, split_y, bounds.width, bounds.y + bounds.height - split_y);
            (left, right)
        } else {
            ...
            let right = Rectangle::new(split_x, bounds.y, bounds.x + bounds.width - split_x, bounds.height);
            (left, right)
        }
    }
    ...
    fn connect_rooms(&self, node: &BSPNode, rng: &mut ChaCha8Rng) -> Vec<Corridor> {
        ...
        if let (Some(left), Some(right)) = (&node.left, &node.right) {
            ...
            if let (Some(left_room), Some(right_room)) = (self.find_closest_room(left, right), self.find_closest_room(right, left)) {
    ...
    fn room_distance(&self, room1: &Room, room2: &Room) -> f32 {
        let center1 = (room1.bounds.x + room1.bounds.width / 2, room1.bounds.y + room1.bounds.height / 2);
        let center2 = (room2.bounds.x + room2.bounds.width / 2, room2.bounds.y + room2.bounds.height / 2);
