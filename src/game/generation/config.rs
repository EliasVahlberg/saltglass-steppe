...
use super::algorithm::{AlgorithmParameters, GenerationError, ParameterValue};
...
impl ConfigurationLoader {
    ...
    pub fn load_from_file<P: AsRef<Path>>(
        path: P,
    ) -> Result<GenerationConfiguration, GenerationError> {
        ...
        for pass in &config.passes {
            for dep in &pass.dependencies {
                if !pass_ids.contains(dep) {
                    return Err(GenerationError::InvalidParameters(format!(
                        "Pass '{}' depends on non-existent pass '{}'",
                        pass.id, dep
                    )));
                }
            }
    ...
    ) -> Result<ParameterValue, GenerationError> {
        match value {
            ...
            _ => Err(GenerationError::InvalidParameters(
...
impl Default for GenerationConfiguration {
    fn default() -> Self {
        Self {
            passes: vec![GenerationPassConfig {
