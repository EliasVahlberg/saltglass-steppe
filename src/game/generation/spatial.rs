...
impl PoissonSampler {
    ...
    pub fn sample_points(&mut self, candidates: &[(i32, i32)], max_points: usize, rng: &mut ChaCha8Rng) -> Vec<(i32, i32)> {
        self.grid.fill(None);
        ...
        while !active.is_empty() && result.len() < max_points {
            ...
            let mut found = false;
            for _ in 0..30 { // Try 30 times to find valid point
    ...
    fn find_valid_candidate(&self, candidates: &[(i32, i32)], center: (i32, i32), rng: &mut ChaCha8Rng) -> Option<(i32, i32)> {
        ...
        for dy in -2..=2 {
            for dx in -2..=2 {
                ...
                if check_x >= 0 && check_y >= 0 && 
                   check_x < self.grid_width as i32 && check_y < self.grid_height as i32 {
...
pub fn distribute_points_grid(candidates: &[(i32, i32)], max_points: usize, min_distance: i32, rng: &mut ChaCha8Rng) -> Vec<(i32, i32)> {
    let mut result = Vec::new();
