...
use super::weighted_table::{WeightedEntry, WeightedTable};
...
impl WorldGenerator {
    ...
    fn generate_connections(&self, seed: u64, pois: &[POI]) -> Vec<Connected> {
        ...
        // Find all towns
        let town_positions: Vec<(usize, usize)> = pois
            .iter()
            .enumerate()
            .filter_map(|(idx, &poi)| {
                if poi == POI::Town {
                ...
                for &connected_town in &connected_towns {
                    for (i, &remaining_town) in remaining_towns.iter().enumerate() {
                        let distance = ((connected_town.0 as i32 - remaining_town.0 as i32).pow(2)
                            + (connected_town.1 as i32 - remaining_town.1 as i32).pow(2))
                            as f64;
    ...
    fn create_road(
        ...
        while current_x != target_x || current_y != target_y {
            ...
            if current_x >= 0
                && current_x < WORLD_WIDTH as i32
                && current_y >= 0
                && current_y < WORLD_HEIGHT as i32
            {
...
mod tests {
    ...
    fn enhanced_poi_placement() {
        ...
        assert!(
            poi_count >= 45,
            "Expected at least 45 POIs with enhanced generation, got {}",
            poi_count
        );
