...
use super::algorithm::{AlgorithmParameters, GenerationAlgorithm, GenerationError, ParameterValue};
...
impl AlgorithmRegistry {
    ...
    pub fn register_algorithm(
        &self,
        algorithm: Arc<dyn GenerationAlgorithm>,
    ...
    pub fn find_by_output_layers(&self, required_layers: &[String]) -> Vec<String> {
        ...
        configurations
            ...
            .filter(|(_, config)| {
                required_layers
                    .iter()
                    .all(|layer| config.supported_layers.output_layers.contains(layer))
            })
            .map(|(id, _)| id.clone())
            .collect()
    }
    ...
    pub fn find_by_performance(
        &self,
        max_speed: SpeedCategory,
        max_memory: MemoryCategory,
    ) -> Vec<String> {
        let configurations = self.configurations.read().unwrap();
        configurations
            .iter()
            .filter(|(_, config)| {
                self.speed_level(&config.performance_profile.speed) <= self.speed_level(&max_speed)
                    && self.memory_level(&config.performance_profile.memory_usage)
                        <= self.memory_level(&max_memory)
            })
            .map(|(id, _)| id.clone())
            .collect()
    }
    ...
    pub fn create_parameters(
        ...
        let config = self.get_config(algorithm_id).ok_or_else(|| {
            GenerationError::InvalidParameters(format!("Unknown algorithm: {}", algorithm_id))
        })?;
    ...
    pub fn get_statistics(&self) -> RegistryStatistics {
        ...
        for config in configurations.values() {
            ...
            *performance_distribution
                .entry(format!("{:?}", config.performance_profile.speed))
                .or_insert(0) += 1;
        }
...
mod tests {
    ...
    fn test_registry_basic_operations() {
        ...
        assert!(
            registry
                .register_algorithm(algorithm.clone(), config)
                .is_ok()
        );
