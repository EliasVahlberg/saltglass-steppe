use crate::game::generation::constraints::{
    ConstraintContext, ConstraintSystem, EntityPlacement, ObjectivePlacement, ResourcePlacement,
};
...
fn create_overworld_context(map: &Map) -> ConstraintContext<'_> {
    ConstraintContext {
        ...
        objectives: vec![ObjectivePlacement {
            objective_type: "landmark".to_string(),
            x: 45,
            y: 35,
            required: true,
        }],
...
fn create_town_context(map: &Map) -> ConstraintContext<'_> {
    ConstraintContext {
        ...
        objectives: vec![
            ObjectivePlacement {
                objective_type: "quest_giver".to_string(),
                x: 5,
                y: 5,
                required: true,
            },
            ObjectivePlacement {
                objective_type: "town_hall".to_string(),
                x: 14,
                y: 14,
                required: true,
            },
...
fn create_arena_context(map: &Map) -> ConstraintContext<'_> {
    ConstraintContext {
        ...
        resources: vec![],
        objectives: vec![ObjectivePlacement {
            objective_type: "survive".to_string(),
            x: 12,
            y: 10,
            required: true,
        }],
...
fn fix_dungeon_constraints(
    mut map: Map,
    results: &[crate::game::generation::constraints::ConstraintResult],
) -> Map {
    for result in results {
        if !result.passed
            && result.severity == crate::game::generation::constraints::ConstraintSeverity::Critical
        {
...
fn print_constraint_results(
    ...
    for result in results {
        ...
        if !result.passed {
            match result.severity {
                crate::game::generation::constraints::ConstraintSeverity::Critical => {
                    critical_failed += 1
                }
                crate::game::generation::constraints::ConstraintSeverity::Warning => {
                    warning_failed += 1
                }
                _ => {}
            }
    ...
    let overall_score =
        crate::game::generation::constraints::ConstraintSystem::calculate_satisfaction_score(
            results,
        );
    println!(
        "    Overall Score: {:.2} | Critical Failed: {} | Warnings Failed: {}",
