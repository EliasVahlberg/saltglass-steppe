    ...
    for config_name in &test_configs {
        ...
        if success {
            ...
            let eval_path = format!(
                "enhanced-tile-test-suite/evaluations/{}_evaluation.json",
                seed
            );

            if Path::new(&eval_path).exists() {
                let eval_content =
                    fs::read_to_string(&eval_path).expect("Failed to read evaluation");
                let evaluation: Value =
                    serde_json::from_str(&eval_content).expect("Failed to parse evaluation");

                let quality_score = evaluation["evaluation"]["quality_score"]
                    .as_f64()
                    .unwrap_or(0.0);
                let empty_constraints = vec![];
                let constraints = evaluation["evaluation"]["constraints"]
                    .as_array()
                    .unwrap_or(&empty_constraints);
                let passed_constraints = constraints
                    .iter()
...
fn generate_test_report(results: &[(String, bool, f64, usize, usize)]) {
    ...
    report.push_str("\n## Quality Metrics\n\n");
    report
        .push_str("- **Quality Score**: Combined connectivity and floor density score (0.0-1.0)\n");
    report.push_str(
