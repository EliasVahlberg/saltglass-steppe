...
fn bsp_algorithm_basic() {
    ...
    for room in &rooms {
        ...
        assert!(
            ...
            "Room exceeds width bounds"
        );
        assert!(
            room.bounds.y + room.bounds.height <= 30,
            "Room exceeds height bounds"
        );
    }
    ...
    for corridor in &corridors {
        ...
        assert!(
            corridor.start.0 <= 40 && corridor.start.1 <= 30,
            "Corridor start out of bounds"
        );
        assert!(
            corridor.end.0 <= 40 && corridor.end.1 <= 30,
            "Corridor end out of bounds"
        );
    }

    println!(
        "✅ BSP Algorithm: Generated {} rooms, {} corridors",
        rooms.len(),
        corridors.len()
    );
}
...
fn cellular_automata_algorithm_basic() {
    ...
    use saltglass_steppe::game::generation::structures::algorithms::{
        CellularAutomataAlgorithm, CellularAutomataParams,
    };
    ...
    for (x, y) in &walls {
        assert!(
            *x >= bounds_check.x as i32 && *x < (bounds_check.x + bounds_check.width) as i32,
            "Wall x out of bounds: {}",
            x
        );
        assert!(
            *y >= bounds_check.y as i32 && *y < (bounds_check.y + bounds_check.height) as i32,
            "Wall y out of bounds: {}",
            y
        );
    }
    ...
    println!(
        "✅ Cellular Automata Algorithm: Generated {} wall tiles",
        walls.len()
    );
}
...
fn run_all_scenarios() {
    ...
    for entry in entries {
        ...
        if path.extension().map(|e| e == "json").unwrap_or(false) {
            scenarios
                .push(Scenario::from_file(&path).expect(&format!("Failed to parse {:?}", path)));
        }
    }
